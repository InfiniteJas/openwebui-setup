<script lang="ts">
	...
	const checkOauthCallback = async () => {
		...
		const sessionUser = await getSessionUser(tkn).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
		if (!sessionUser) {
						...
						>
							...
							{#if $config?.features.enable_login_form || $config?.features.enable_ldap}
								<div class="flex flex-col mt-4">
									{#if mode === 'signup'}
										<div class="mb-2">
											<div class="px-2 text-sm font-medium text-left mb-1">{$i18n.t('Name')}</div>
											<input
												bind:value={name}
												type="text"
									...
									{#if mode === 'ldap'}
										<div class="mb-2">
											<div class="px-2 text-sm font-medium text-left mb-1">
												{$i18n.t('Username')}
											</div>
											<input
									...
									{:else}
										<div class="mb-2">
											<div class="px-2 text-sm font-medium text-left mb-1">{$i18n.t('Email')}</div>
											<input
												bind:value={email}
												type="email"
												class="px-2 py-1 my-0.5 w-full text-sm outline-hidden bg-transparent"
									...
									<div>
										<div class="px-2 text-sm font-medium text-left mb-1">{$i18n.t('Password')}</div>

										<input
											bind:value={password}
											type="password"
											class="px-2 py-1 my-0.5 w-full text-sm outline-hidden bg-transparent"
											placeholder={$i18n.t('Enter Your Pawd')}
							...
							<div class="px-3 mt-4 flex items-start text-left">
								<input
									type="checkbox"
