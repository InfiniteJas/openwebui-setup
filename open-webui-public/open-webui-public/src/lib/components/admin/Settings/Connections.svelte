<script lang="ts">
	...
	const updateOpenAIHandler = async () => {
		if (ENABLE_OPENAI_API !== null) {
			...
			if (OPENAI_KS.length !== OPENAI_API_BASE_URLS.length) {
				// if there are more keys than urls, remove the extra keys
				if (OPENAI_KS.length > OPENAI_API_BASE_URLS.length) {
					OPENAI_KS = OPENAI_KS.slice(0, OPENAI_API_BASE_URLS.length);
				}

				// if there are more urls than keys, add empty keys
				if (OPENAI_KS.length < OPENAI_API_BASE_URLS.length) {
					const diff = OPENAI_API_BASE_URLS.length - OPENAI_KS.length;
	...
	const addOpenAIConnectionHandler = async (connection) => {
		...
		OPENAI_KS = [...OPENAI_KS, connection.key];
	...
	onMount(async () => {
		if ($user.role === 'admin') {
			...
			OPENAI_KS = openaiConfig.OPENAI_API_KEYS;
								...
								{#each OPENAI_API_BASE_URLS as url, idx}
									<OpenAIConnection
										...
										onDelete={() => {
											...
											OPENAI_KS = OPENAI_KS.filter((key, keyIdx) => idx !== keyIdx);
