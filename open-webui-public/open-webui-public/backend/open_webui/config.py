...
for file_path in (FRONTEND_BUILD_DIR / "static").glob("**/*"):
    if file_path.is_file():
        ...
        try:
            ...
            logging.error("An error occurred during file copy operation")
...
if frontend_favicon.exists():
    try:
        ...
        logging.error(f"Favicon copy failed:")
...
# SAFE_NAME_RE = re.compile(r"^[\w\-]{1,50}$")

# if CUSTOM_NAME and SAFE_NAME_RE.fullmatch(CUSTOM_NAME):
#     try:
#         r = requests.get(f"https://api.openwebui.com/api/v1/custom/{CUSTOM_NAME}", timeout=3)
#         data = r.json()
#         if r.ok:
#             if "logo" in data:
...
#         pass
# else:
#     log.warning("Invalid CUSTOM_NAME value or not set.")
...
if not OPENAI_API_KEY:
    log.warning("OPENAI_API_KEY is not set. Some features may not work.")
