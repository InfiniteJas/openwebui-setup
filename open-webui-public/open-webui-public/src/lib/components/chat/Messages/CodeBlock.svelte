<script lang="ts">
	...
	const render = async () => {
		if (lang === 'mermaid' && (markdownTkn?.raw ?? '').slice(-4).includes('```')) {
			(async () => {
	...
	$: if (markdownTkn) {
		if (JSON.stringify(markdownTkn) !== JSON.stringify(_renderTkn)) {
			_renderTkn = markdownTkn;
		}
	}
