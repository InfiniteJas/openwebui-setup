<script lang="ts">
	...
	let APIK = ''; //APIKey
	let APIKCopied = false; //APIKeyCopied
	...
	const createAPIKeyHandler = async () => {
		APIK = await createAPIKey(localStorage.token);
		if (APIK) {
	...
	onMount(async () => {
		...
		APIK = await getAPIKey(localStorage.token).catch((error) => {
			console.log(error);
			return '';
		});
	});
			...
			<div class="flex flex-col gap-4">
				...
				{#if $config?.features?.enable_api_key ?? true}
					<div class="justify-between w-full">
						...
						<div class="flex mt-2">
							{#if APIK}
								<SensitiveInput value={APIK} readOnly={true} />

								<button
									class="ml-1.5 px-1.5 py-1 dark:hover:bg-gray-850 transition rounded-lg"
									on:click={() => {
										copyToClipboard(APIK);
										APIKCopied = true;
										setTimeout(() => {
											APIKCopied = false;
										}, 2000);
