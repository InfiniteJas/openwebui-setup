...
async def process_web_search(
    ...
    try:
        logging.info("Initiating web search request.")
...
def reset_upload_dir(user=Depends(get_admin_user)) -> bool:
    ...
    try:
        if upload_dir_path.exists():
            for item_path in upload_dir_path.iterdir():
                try:
                    ...
                    try:
                        ...
                        log.warning(f"Blocked attempt to access: {item_resolved}")
                        continue

                    if item_resolved.is_file() or item_resolved.is_symlink():
                        item_resolved.unlink()
            ...
            log.warning(f"The directory {upload_dir_path} does not exist")
            
    except Exception as e:
        log.exception(f"Failed to process the directory {upload_dir_path}. Reason: {e}")
