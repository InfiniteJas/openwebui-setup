            ...
            def tag_content_handler(content_type, tags, content, content_blocks):
                ...
                if content_blocks[-1]["type"] == "text":
                    ...
                    if re.search(end_tag_pattern, content):
                        ...
                        if isinstance(end_tag, str) and 1 <= len(end_tag) <= 32:
                            if not re.match(r'^[a-zA-Z0-9_-]+$', end_tag):
                                raise ValueError("Invalid end_tag format: only alphanumeric characters, underscore and dash allowed")
                            
                            end_tag_pattern = rf"<{re.escape(end_tag)}>"
                            if isinstance(end_tag_pattern, str) and len(end_tag_pattern) <= 64:
                                try:
