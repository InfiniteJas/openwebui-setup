...
async def show_model_info(
    ...
    urls = models[form_data.name]["urls"]
    index = hash(time.time_ns()) % len(urls)
    url_idx = index
...
async def get_ollama_url(request: Request, model: str, url_idx: Optional[int] = None):
    if url_idx is None:
        ...
        urls = models[model].get("urls", [])
        if not urls:
            raise ValueError(f"No URLs found for model: {model}")

        url_idx = urls[hash(time.time_ns()) % len(urls)]
    url = request.app.state.config.OLLAMA_BASE_URLS[url_idx]
    return url, url_idx
        ...
        async with session.get(file_url, headers=headers) as response:
            ...
            with path_obj.open("ab+") as file:
                ...
                if done:
                    ...
                    response = requests.post(url, data=file, timeout = 6)
...
async def upload_model(
    ...
    try:
        file_path.resolve().relative_to(upload_dir_path.resolve())
    except ValueError:
        raise Exception("Unsafe file directory path")
