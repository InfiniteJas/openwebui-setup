...
ALLOWED_EMBEDDING_HOSTS = {"api.openai.com", "localhost", "127.0.0.1"}

def is_allowed_url(url: str) -> bool:
    parsed = urlparse(url)
    return parsed.scheme in {"http", "https"} and parsed.hostname in ALLOWED_EMBEDDING_HOSTS
...
def generate_openai_batch_embeddings(
    ...
    try:
        if not is_allowed_url(url):
            raise ValueError(f"Blocked unsafe embedding URL: {url}")
