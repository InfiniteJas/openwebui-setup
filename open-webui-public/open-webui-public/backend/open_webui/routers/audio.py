...
def transcribe(request: Request, file_path):
    ...
    if request.app.state.config.STT_ENGINE == "":
        ...
        if is_mp4_audio(file_path):
            ...
            mp4_file_path = original_file_path.with_suffix('.mp4')
            
            try:
                original_file_path.resolve().relative_to(cache_dir_path.resolve())
                mp4_file_path.resolve().relative_to(cache_dir_path.resolve())
            except ValueError:
                raise Exception("Unsafe rename paths")

            original_file_path.rename(mp4_file_path)
            
            # Convert MP4 audio file to WAV format
            convert_mp4_to_wav(str(mp4_file_path), str(original_file_path))
        ...
        try:
            ...
            try:
                file_path_obj.resolve().relative_to(file_dir_path.resolve())
            except ValueError:
            ...
            with transcript_file_path.open("w") as f:
                json.dump(data, f)
        ...
        if not request.app.state.config.TTS_OPENAI_API_BASE_URL.startswith(
            ...
            try:
                response = requests.get(
                    f"{request.app.state.config.TTS_OPENAI_API_BASE_URL}/audio/models", timeout=30
                )
...
def get_available_voices(request) -> dict:
    ...
    if request.app.state.config.TTS_ENGINE == "openai":
        ...
        if not request.app.state.config.TTS_OPENAI_API_BASE_URL.startswith(
            ...
            try:
                response = requests.get(
                    f"{request.app.state.config.TTS_OPENAI_API_BASE_URL}/audio/voices", timeout=30
                )
                response.raise_for_status()
                data = response.json()
