...
async def update_password(
    ...
    if session_user:
        ...
        if user:
            ...
            raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_PAROL)
    else:
        raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
...
async def signin(request: Request, response: Response, form_data: SigninForm):
    if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        admin_password = os.getenv("WEBUI_ADMIN_PASSWORD", "admin")
...
async def update_admin_config(
    ...
    if isinstance(form_data.JWT_EXPIRES_IN, str) and len(form_data.JWT_EXPIRES_IN) <= 32:
        if re.fullmatch(pattern, form_data.JWT_EXPIRES_IN):
